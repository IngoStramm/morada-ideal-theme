jQuery(function ($) {
    ...
    const defaultPrecosArray = ajax_object.default_prices;
    const selectedPrecosArray = ajax_object.selected_prices;

    const defaultMinPrice = defaultPrecosArray[0];
    const defaultMaxPrice = defaultPrecosArray[defaultPrecosArray.length - 1];
    const selectedMinPrice = selectedPrecosArray[0];
    const selectedMaxPrice = selectedPrecosArray[selectedPrecosArray.length - 1];

    const defaultAreasArray = ajax_object.default_areas;
    const selectedAreasArray = ajax_object.selected_areas;

    const defaultMinArea = defaultAreasArray[0];
    const defaultMaxArea = defaultAreasArray[defaultAreasArray.length - 1];
    const selectedMinArea = selectedAreasArray[0];
    const selectedMaxArea = selectedAreasArray[selectedPrecosArray.length - 1];


    function miSetupRangeSlider(rangeSlider, rangeMinValue, rangeMaxValue, startMinValue, startMaxValue, postfix = '€', decimals = 0, thousand = ',') {

        if (typeof rangeSlider === undefined || !rangeSlider) {
            return;
        }

        if (!startMinValue) {
            startMinValue = rangeMinValue;
        }

        if (!startMaxValue) {
            startMaxValue = rangeMaxValue;
        }
        ...
        rangeSlider.noUiSlider.on('update', function (values, handle) {
            const sliderLabels = rangeSlider.nextElementSibling;
            const minValue = values[0];
            const formattedMinValue = moneyFormat.from(minValue);
            const maxValue = values[1];
            const formattedMaxValue = moneyFormat.from(maxValue);
            if (sliderLabels.classList.contains('slider-labels')) {
                const dataMinValueInput = sliderLabels.querySelector('[data-min-value]');
                const dataMaxValueInput = sliderLabels.querySelector('[data-max-value]');
                dataMinValueInput.value = formattedMinValue;
                dataMaxValueInput.value = formattedMaxValue;
            }
            const boxTitlePrice = rangeSlider.previousElementSibling;
            if (boxTitlePrice.classList.contains('box-title-range')) {
                const dataMinCaptionSpan = boxTitlePrice.querySelector('[data-min-caption]');
                const dataMaxCaptionSpan = boxTitlePrice.querySelector('[data-max-caption]');
                dataMinCaptionSpan.innerHTML = minValue;
                dataMaxCaptionSpan.innerHTML = maxValue;
            }
        });
    }
    ...
    $(document).ready(function () {
        ...
        if (typeof rangeSlider !== undefined && rangeSlider) {
            ...
            miSetupRangeSlider(rangeSlider, defaultMinPrice, defaultMaxPrice, selectedMinPrice, selectedMaxPrice, ' €');
        }
    });
    ...
    $(document).ready(function () {
        ...
        const rangeSlider = document.getElementById('slider-range2');

        miSetupRangeSlider(rangeSlider, defaultMinArea, defaultMaxArea, selectedMinArea, selectedMaxArea, ' m²');
    });
    ...
    (function (factory) {
        ...
        function initialize(target, originalOptions) {
            if (typeof target === undefined || !target || !target.nodeName) {
                return;
