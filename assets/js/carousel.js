jQuery(function ($) {
    ...
    const thumbsSwPagi = document.querySelectorAll('.thumbs-sw-pagi');
    let pagithumbs = null;

    if (thumbsSwPagi.length > 0) {
        pagithumbs = new Swiper(".thumbs-sw-pagi", {
            spaceBetween: 14,
            slidesPerView: "auto",
            freeMode: true,
            watchSlidesProgress: true,
    ...
    if (pagithumbs) {
        swiperSingleArgs.thumbs = {
            swiper: pagithumbs,
        };
    }

    var swiperSingle = new Swiper(".sw-single", swiperSingleArgs);
